(function(root, factory) {
  ...
  (function(window, document, Chartist) {
    ...
    Chartist.getAvailableHeight = function(svg, options) {
      return Math.max((Chartist.quantity(options.height).value || svg.height()) - (options.chartPadding.top + options.chartPadding.bottom) -
      ...
      return function cardinal(pathCoordinates, valueData) {
        ...
        if (segments.length > 1) {
          ...
          for (var i = 0, iLen = pathCoordinates.length; iLen - 2 * !z > i; i += 2) {
            ...
            path.curve(
              (t * (-p[0].x + 6 * p[1].x + p[2].x) / 6) + (c * p[2].x), (t * (-p[0].y + 6 * p[1].y + p[2].y) / 6) + (c * p[2].y), (t * (p[
                1].x + 6 * p[2].x - p[3].x) / 6) + (c * p[2].x), (t * (p[1].y + 6 * p[2].y - p[3].y) / 6) + (c * p[2].y),
              p[2].x,
              p[2].y,
              false,
              valueData[(i + 2) / 2]
  ...
  (function(window, document, Chartist) {
    ...
    function AutoScaleAxis(axisUnit, data, chartRect, options) {
      ...
      this.bounds = Chartist.getBounds(chartRect[axisUnit.rectEnd] - chartRect[axisUnit.rectStart], highLow, options.scaleMinSpace || 20,
        options.onlyInteger);
      this.range = {
        min: this.bounds.min,
        max: this.bounds.max
  ...
  (function(window, document, Chartist) {
    ...
    function createChart(options) {
      var data = {
        raw: this.data,
        normalized: options.distributeSeries ? Chartist.getDataArray(this.data, options.reverseData, options.horizontalBars ? 'x' : 'y').map(
          function(value) {
            return [value];
        ...
        data.normalized[seriesIndex].forEach(function(value, valueIndex) {
          ...
          if (options.horizontalBars) {
            projected = {
              ...
              y: chartRect.y1 - labelAxis.projectValue(value && value.y ? value.y : 0, labelAxisValueIndex, data.normalized[
                seriesIndex])
            };
          } else {
            projected = {
              x: chartRect.x1 + labelAxis.projectValue(value && value.x ? value.x : 0, labelAxisValueIndex, data.normalized[
                seriesIndex]),
          ...
          if (labelAxis instanceof Chartist.StepAxis) {
            ...
            projected[labelAxis.units.pos] += (options.stackBars || options.distributeSeries) ? 0 : biPol * options.seriesBarDistance *
  ...
  (function(window, document, Chartist) {
    ...
    function createChart(options) {
      ...
      // Create SVG.js draw
      this.svg = Chartist.createSvg(this.container, options.width, options.height, options.donut ? options.classNames.chartDonut : options.classNames
        .chartPie);
